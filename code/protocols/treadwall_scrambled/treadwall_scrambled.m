...
% load parameters
params_file = fullfile(treadwall_paramsdir(), 'treadwall_scrambled_parameters.m');
run(params_file)
...
for currentTrial = 1:S.GUI.MaxTrialNumber
    ...
    % read output action
    stimOutput = get_stimoutput(triallist{currentTrial});
    ...
    if currentTrial == 1
        ...
        sma = AddState(sma, 'Name', 'StopCamera', ...
            'Timer', cfg.bpod.stop_camera_delay_s,...
