...
frames_folder = os.path.join(nvme_session_path, f"{animal_name}_{session_name}_topcam_frames")
timestamp_filename = os.path.join(nvme_session_path, f"{animal_name}_{session_name}_topcam_video_timestamps.txt")
    ...
    if user_input == 'y':
        ...
        while os.path.exists(frames_folder):
            frames_folder = os.path.join(
                nvme_session_path,
                f"{animal_name}_{session_name}_topcam_frames_{counter}"
            )
            timestamp_filename = os.path.join(
                nvme_session_path,
                f"{animal_name}_{session_name}_topcam_{counter}_video_timestamps.txt"
            )
            counter += 1
